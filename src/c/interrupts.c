...
// Subroutines for the first 32 interrupts (CPU exceptions)

/* exception_0_DE_handler:
 *      Handler for the division errors exception,
 *      only calls the panic path now
 *
 *      @param *states  The stack pointer to the registers saved
 */
void exception_0_DE_handler(interrupt_stack_state_t *states)
{
        panic(PANIC_DIVIDE_BY_ZERO, states);
}

/* exception_6_UD_handler:
 *      Handler for the invalid opcode exception,
 *      only calls the panic path now
 *
 *      @param *states  The stack pointer to the registers saved
 */
void exception_6_UD_handler(interrupt_stack_state_t *states)
{
        panic(PANIC_INVALID_OPCODE, states);
}

/* exception_8_DF_handler:
 *      Handler for the double fault exception,
 *      only calls the panic path now 
 *
 *      @param *states  The stack pointer to the registers saved
 */
void exception_8_DF_handler(interrupt_stack_state_t *states)
{
        panic(PANIC_DOUBLE_FAULT, states);
}

/* exception_13_GP_handler:
 *      Handler for the general protection exception,
 *      only calls the panic path now
 *
 *      @param *states  The stack pointer to the registers saved
 */
void exception_13_GP_handler(interrupt_stack_state_t *states)
{
        panic(PANIC_GENERAL_PROTECTION_FAULT, states);
}
 ...
 *
 *      Each subroutine for its corresponding interrupt has its offset from the
 *      IDT marked as comment
 *
 *      @param  *stack  The state of registers, interrupt number, error code,
 *                      the EIP register, the cs register and eflags pushed onto the stack
 *                      for safe return
 */
void interrupt_handler(interrupt_stack_state_t *frame){
        switch (frame->interrupt) {
                case 0:
                        exception_0_DE_handler(frame); // Offset 0x0
                        break;
                case 1:
                        break;
                case 2:
                ...
                case 5:
                        break;
                case 6:
                        exception_6_UD_handler(frame); // Offset 0x6
                        break;
                case 7:
                        break;
                case 8:
                        exception_8_DF_handler(frame); // Offset 0x8
                        break;
                case 9:
                        break;
                case 10:
                ...
                case 12:
                        break;
                case 13:
                        exception_13_GP_handler(frame); // Offset 0xD
                        break;
                case 14:
                        break;
                case 15:
                ...
                case 32:
                        break;
                case 33:
                        irq_1_keyboard_interrupt_handler(); // Offset 0x21
                        break; 
        }
